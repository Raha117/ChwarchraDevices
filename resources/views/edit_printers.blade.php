                ...
                <tbody>
                    @foreach ($printers as $print)
                        <tr>
                            ...
                            @if($print->id != $id)
                                @foreach ($print->device_specifications as $specification)
                            ...
                            @else
                            <form method="post" action="/printers/{{$id}}">
                                @csrf
                                @method('patch')
                                @foreach( $specs as $spec )
                                    @if($spec != 'Active')
                                        <td><input name="{{$spec}}" value="{{ucfirst($printer_spec_values[$loop->iteration -1 ])}}" type="text" class="input-md md no-margin" id="{{$spec}}" placeholder="{{ucfirst($spec)}}"></td>
                                    @else
                                        <td>
                                            <select name="active" class="">
                                                <option disabled>Active</option>
                                                @if($printer_spec_values[4] == 'Yes')
